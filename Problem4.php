...
<html lang="en">
<head>
  <title>Question4 for My Passion Media Inc</title>
  <meta charset="utf-8">
	...
	<div class="container">
		...
		<?php
			...
			*/
			...
			//Creating a class called Products from which we get the get and set the dimensions of each products
			class Products {
			...
			//Putting the given products in an associative array
			$productsArray = array(
				"Fiddle" => array(
					"weight" => 1,
					"dimension" => array(60,20,10)
				),
				"Dish" => array(
					"weight" => 0.1,
					"dimension" => array(30,30,5)
				),
				"Spoon" => array(
					"weight" => 0.05,
					"dimension" => array(15,5,2)
				)
			);
			
			//Getting the input from the form and adding the new product into the array $productsArray
			if($_SERVER["REQUEST_METHOD"] == "POST") {
				...
				$Width = $_POST['width'];
				$Height = $_POST['height'];
				$productsArray += array($ProductName => array("weight" => $Weight,"dimension" => array($Length,$Width,$Height)));
			}
			
			//
			/*Adding inputs from the array using foreach loop into HTML  selection option*/
			echo "
			<div class='row'>
				<div class='col-sm-12'>
					<form>
						...
						foreach($productsArray as $productID=> $attributes) {
			...
			/*looping through the array to display the product and its attributes*/
			?><div class="col-sm-12" style = "display:block" id="myDIV">
			<?php	
			if(isset($_GET["mySelect"]) == " " || isset($_GET["mySelect"])){
				foreach($productsArray as $productID => $attributes) {
					$product = new Products($productID,$attributes['weight'],array($attributes['dimension'][0],$attributes['dimension'][1],$attributes['dimension'][2]));
					if($productID == $_GET["mySelect"] ){
						?>	
						<div>
			...
			/*End of looping through the array to display the product and its attributes*/
			?>	
			</div>
			<!--Form for the new product entry-->
			<button type="button" class="btn btn-success" onclick="myFunction()">Add Product</button>
